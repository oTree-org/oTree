...
class Group(BaseGroup):
    ...
    def reforms(self):
        reforms_number = self.get_player_by_id(1).reforms_vote
        for p in self.get_players():
            if self.reforms_number != p.reforms_vote:
                self.reforms_number = 0
            else:
                pass
        return self.reforms_number
    ...
    def payoffs(self):
        for p in self.get_players():
            ...
            # Я ОСТАНОВИЛСЯ НА ФУНКЦИИ НЕТУ РЕФОРМ КАК СЧИТАТЬ PAYOFF КАК ИМЕННО ПРОХОДЯТ РЕФОРМЫ ТО ЕСТЬ КАЖДОМУ ИГРОКУ ИЛИ ЧТО


class Player(BasePlayer):

    reforms_vote = models.FloatField(widget=widgets.SliderInput(attrs={'step': '1'}), min=0, max= 5, default=3)